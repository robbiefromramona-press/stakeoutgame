...
| Player UI | `#screen-player` | Two live canvas gauges, the numeric readouts, the level stopwatch and ten touch hotspots (two D-pads + two MEASURE discs), built from the V12 layered art in `assets/img/v12/` (generated by `tools/prep_v12_layers.py`) |
...
That took the V11 art from 5.5 MB to 655 KB (88% smaller). The player screen's `with_measure_and_clock` image was retired in V12, replaced by the layered art in `assets/img/v12/`:
